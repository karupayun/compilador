...
fun member _ [] = false
| member y (x::xs)  = if x=y then true else (member y xs)  

fun elimSR l = List.filter (fn x => not (member x tigerframe.specialregs)) l
         ...
         | mktln (i::inss) tl tn = mktln inss tl (tn @ [(newNode(gc), i)] )
	       ...
	       fun mkgr [] d u im = FGRAPH {control = gc, def = d, use = u, ismove = im}
             | mkgr ((n,OPER{src,dst, jump,...})::xs) d u im = let 
              ...
              | mkgr ( (n,MOVE{src,dst, ...})::xs) d u im = if not(member src tigerframe.specialregs) andalso not(member dst tigerframe.specialregs) then let 
                                                                val d' = Splaymap.insert(d,n,[dst]) (*aca lo mismo*)
